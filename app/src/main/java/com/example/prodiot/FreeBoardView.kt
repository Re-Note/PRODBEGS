...
class FreeBoardView : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val context = applicationContext // 이 부분에 해당하는 context를 적절히 대체해야 합니다.
        val sharedPreferences = context.getSharedPreferences("selectedComment", Context.MODE_PRIVATE)
        var isReply = sharedPreferences.getBoolean("isReply", false)
        isReply = false
        Log.d("초기화전","$isReply")
        ...
        // 이 부분은 초기화시와 값이 변경될 때 모두 호출되도록 로직을 작성합니다.
        fun updateTextView(isReply: Boolean) {
            Log.d("함수 내부","$isReply")
            if (isReply) {
                commentEditText.hint = "대댓글 작성"
                Toast.makeText(this@FreeBoardView, "대댓글 작성 상태", Toast.LENGTH_SHORT).show()
            } else {
...
// 값 변경 시점
        ...
        btn_comment.setOnClickListener {
            var updatedIsReply2 = sharedPreferences.getBoolean("isReply", false)
            Log.d("작성버튼","$updatedIsReply2")
            if (updatedIsReply2) {
    ...
    override fun onBackPressed() {
        val context = applicationContext // 이 부분에 해당하는 context를 적절히 대체해야 합니다.
        val sharedPreferences10 = context.getSharedPreferences("selectedComment", Context.MODE_PRIVATE)
        val isReply = sharedPreferences10.getBoolean("isReply", false)
        if (isReply) {
            Log.d("백 버튼 활성화","$isReply")
            commentEditText.hint = "댓글 작성"
            Toast.makeText(this@FreeBoardView, "댓글 작성 상태", Toast.LENGTH_SHORT).show()
            val editor = sharedPreferences10.edit()
            editor.putBoolean("isReply", false)
            editor.apply()
        } else {
            super.onBackPressed() // 기본 동작 수행 (이전 화면으로 이동)
        }
    }
        ...
        databaseReference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                post?.let {
                    ...
                    CreatedAtView.text = formattedTimestamp
                ...
                val sharedPref5 = getSharedPreferences("post_data", Context.MODE_PRIVATE)
                val editor5 = sharedPref5.edit()
                editor5.putString("LangString", post?.language)
                Log.d("language", "Output Text: ${post?.language}")
                editor5.apply()
            }
                ...
                override fun onDataChange(commentsSnapshot: DataSnapshot) {
                    ...
                    for (commentSnapshot in commentsSnapshot.children) {
                        ...
                        commentKey?.let {
                            // 대댓글 삭제를 위한 쿼리 생성
                            val repliesQuery =
                                repliesRef.orderByChild("parentkey").equalTo(commentKey)
                            repliesQuery.addListenerForSingleValueEvent(object :
                                ValueEventListener {
